...
For votes = [2, 3, 5, 2] and k = 3, the output should be
  ...
  The first candidate got 2 votes. Even if all of the remaining 3 candidates vote for him, he will still have only 5 votes, i.e. the same
  number as the third candidate, so there will be no winner.
  The second candidate can win if all the remaining candidates vote for him (3 + 3 = 6 > 5).
  The third candidate can win even if none of the remaining candidates vote for him. For example, if each of the remaining voters cast
  their votes for each of his opponents, he will still be the winner (the votes array will thus be [3, 4, 5, 3]).
...
def electionsWinners(votes, k):
    maximum = max(votes)
    numberMaxVotes = votes.count(maximum)
    count = 0
    if k == 0 and numberMaxVotes > 1:
        return 0
    if k == 0 and numberMaxVotes == 1:
        return 1
    for i in votes:
